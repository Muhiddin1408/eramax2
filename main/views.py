...
def FilialKirim(request):
    ...
    for filial in filials:
        response.append({"name": f"{filial}", "data": [round(Shop.objects.filter(filial=filial, date__year=year, date__month=i).aggregate(foo=Coalesce(
...
class Home(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        jami = shops.aggregate(foo=Coalesce(
...
class ProductShopView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if filial_name is None:
            products = products.filter(filial=1)
...
def get_savdo_tahlil(request):
    ...
    total_qoldiq_sotish_dollar = sum(
        branch['jami_qoldiq_sotish_dollar'] for branch in data_for_table)
...
def chart_savdo_tahlil(request):
    ...
    get_sum = Sum(F("naqd_dollar")+F("transfer")+F("plastik") +
                  F("nasiya_dollar")-F("skidka_dollar"), output_field=IntegerField())
    ...
    get_arrival_sum = Sum(F("arrival_price")*F("quantity"),
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                form.add_error(
                    None, f"Xatolik mavjud: {e} Ma'lumotlar to'g'riligiga ishonch xosil qiling va sayt dasturchilariga bog'laning!")
