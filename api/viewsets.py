...
class ReceiveItemViewset(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if product.dollar != valid_data.get("dollar") or product.sotish_dollar != valid_data.get("sotish_dollar")or product.som != valid_data.get("som") or product.sotish_som != valid_data.get(
                "sotish_som") or product.expired_date != valid_data.get("expired_date"):
            if ProductFilial.objects.filter(barcode=barcode, expired_date=valid_data.get("expired_date")):
                product = ProductFilial.objects.get(barcode=barcode, expired_date=valid_data.get("expired_date"))
    ...
    def by_faktura(self, request):
        product1 = ProductFilial.objects.get(id=request.GET.get("product"))
...
class FakturaItemViewset(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        product_filial = ProductFilial.objects.filter(filial_id=valid_data.get("faktura").filial_id, barcode=product.barcode)
        diff = 0
...
class ShopViewset(viewsets.ModelViewSet):
    ...
    def total_shop(self, request):
        ...
        avg_sum = shops.annotate(total_sum=(F('naqd_dollar')+F('plastik')+F('nasiya_dollar')+F('transfer')-F('skidka_dollar')))\
        ...
        if request.method == 'POST':
            try:
                ...
                with transaction.atomic():
                    ...
                    if cart is not None:
                        for c in cart:
                            product = ProductFilial.objects.get(barcode=c['barcode'], filial_id=filial)
...
class HisobdanChiqarishOmborViewset(viewsets.ModelViewSet):
    ...
    def by_id(self, request):
        ...
        quantity = float(resive_sum) - float(request.GET.get('quantity'))
        ReceiveItem.objects.filter(product_id=product).update(quantity=quantity)
        b = ProductFilial.objects.filter(id=product)
...
class QaytganProductOmborView(viewsets.ModelViewSet):
    ...
    def by_id(self, request):
        # id = int(request.GET.get('f'))
        product = int(request.GET.get('product'))
        resive = ReceiveItem.objects.filter(product_id=product, receive__status=1)
        faktura_item = FakturaItem.objects.all()
        print(resive)
        print(resive)
        resive_sum = resive.aggregate(foo=Coalesce(
            Sum('quantity'),
        ...
        quantity = float(resive_sum)-float(request.GET.get('quantity'))
        ReceiveItem.objects.filter(product_id=product).update(quantity=quantity)
        b = ProductFilial.objects.filter(id=product, ombor=True)
        serializer = self.get_serializer(b, many=True)
        return Response(serializer.data)
